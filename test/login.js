loginTest = require("../pages/pomLogin")
loginData = require("../data/loginData")
inventoryTest = require("../pages/pomInventory")
cartTest = require("../pages/pomCart")
productDetailsTest = require("../pages/pomProductDetails")
checkoutOneTest = require("../pages/pomCheckoutOne")
checkoutOneData = require("../data/checkOutPageOneData") 
checkoutTwoTest = require("../pages/pomCheckoutTwo")
checkoutCompleteTest = require("../pages/pomCheckoutComplete")
...
describe ("'Login' page actions", function(){
    ...
    it("should verify that if warning text is shown when 'Password' field is empty in 'Login' page",()=>{
        loginTest.enterUserNameLoginPage(loginData.loginUserNameData)
        assert.equal(loginData.loginUserNameData,loginTest.loginUserName.getValue())
        assert.equal('',loginTest.loginPassword.getValue())
        loginTest.clickLoginBtnLoginPage()
    ...
    it("should verify that if warning text is shown when wrong username is sent for the 'Username' field in 'Login' page",()=>{
        ...
        assert.equal('test',loginTest.loginUserName.getValue())
        loginTest.loginPassword.clearValue()
        loginTest.enterPasswordLoginPage(loginData.loginPasswordData)
        assert.equal(loginData.loginPasswordData,loginTest.loginPassword.getValue())
        loginTest.clickLoginBtnLoginPage()
        expect(loginTest.warningTextLoginPage()).equals('Epic sadface: Username and password do not match any user in this service')        
    })
...
describe ("'Checkout One' page actions",function(){
    ...
    it ("should verify that if warning message shown when lastname and postal/zipcode is not available for the 'Last Name' and 'Zip/Postal Code' fields respectively and 'CONTINUE' button has been clicked from 'Checkout One' page", () =>{
        checkoutOneTest.enterFirstNameCheckoutOne(checkoutOneData.checkoutFirstNameData)
        assert.equal(checkoutOneData.checkoutFirstNameData,checkoutOneTest.checkoutOneFirstName.getValue())
        checkoutOneTest.clickContinueBtnCheckoutOnePage()
        checkoutOneTest.checkoutOneWarningText.waitForDisplayed()
    ...
    it ("should verify that if warning message is shown when postal/zipcode is not available for the 'Zip/Postal Code' field and 'CONTINUE' button has been clicked from 'Checkout One' page", () =>{
        checkoutOneTest.enterLastNameCheckoutOne(checkoutOneData.checkoutPageLastNameData)
        assert.equal(checkoutOneData.checkoutPageLastNameData,checkoutOneTest.checkoutOneLastName.getValue())
        checkoutOneTest.clickContinueBtnCheckoutOnePage()
        checkoutOneTest.checkoutOneWarningText.waitForDisplayed()
    ...
    it ("should verify that if 'CONTINUE' button has been clicked from 'Checkout One' page and moved to 'Checkout Two' page", () =>{
        checkoutOneTest.enterPostalORZipCheckoutOne(checkoutOneData.checkoutPostalOrZipCodeData)
        assert.equal(checkoutOneData.checkoutPostalOrZipCodeData,checkoutOneTest.checkoutOnePostalOrZipCode.getValue())
        checkoutOneTest.clickContinueBtnCheckoutOnePage()
